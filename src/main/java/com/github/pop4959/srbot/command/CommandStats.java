    ...
    public void execute(MessageReceivedEvent event, String[] args) {
        if (args.length > 0) {
            ...
            try {
                ...
                event.getChannel().sendMessage(EmbedTemplates.empty(event.getGuild()).setDescription(groupforCodes(20)).addField("Movement", groupforCodes(21, 22, 23, 24), true).addField("Sliding", groupforCodes(25, 26), true).addField("Grappling", groupforCodes(27, 28), true).addField("Golden Hook", groupforCodes(0, 1, 2), true).addField("Bomb", groupforCodes(3, 4, 5), true).addField("Shockwave", groupforCodes(6, 7, 8), true).addField("Crate", groupforCodes(9, 10, 11, 12, 13), true).addField("Rocket", groupforCodes(14, 15, 16), true).addField("Drill", groupforCodes(17, 18, 19), true).setAuthor("Stats for " + steamProfile.getName(), steamProfile.getProfileUrl(), steamProfile.getAvatarFullUrl()).build()).queue();
